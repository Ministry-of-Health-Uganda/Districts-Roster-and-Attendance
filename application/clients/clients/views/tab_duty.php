...
// if beyond 20 days from start date, disable editing or for other month for non system admins
$today=date("d");
...
//Startpoint
$startpoint=date('Y')."-".$thismonth."-01 first day of last month";

$lastmonth=date_create($startpoint);

$convertedstart= $lastmonth->format('Y-m')."-".$startdate;

$lowerlimit=strtotime($convertedstart);


//Endpoint
$endpoint=date('Y')."-".$thismonth."-".$deadline;

$upperlimit=strtotime($endpoint);

//Today

$todaydate=$year."-".$month."-".$today;

$selecteddate=strtotime($todaydate);


 $daysbackwards=round(($selecteddate-$lowerlimit)/ (60 * 60 * 24)); //lowest value is -20
 
 $daysforwards=round(($selecteddate-$upperlimit)/ (60 * 60 * 24)); //greatest value is 20


// 20 is the number of days allowed bettween start and end point for modifying data/entering data

if( (($daysbackwards>20 or $daysbackwards<0) and ($daysforwards<-20 or ($daysforwards>0 or $daysforwards>20)))&& $_SESSION['role']!=='sadmin'){
