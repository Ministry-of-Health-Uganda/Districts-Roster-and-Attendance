...
if($district){
   ...
   //<option disabled >--Select Facility--</option>
	...
	foreach ($facility_types as $type) {
		...
		$type_id=str_replace("facility_type|", "", $type->type_id);
...
<div class="form-group">
    ...
    <center>
    <a class="btn" style="width:100%; background-color:#4bcf23;" href="<?php echo base_url(); ?>jobs/printEstablishments/<?=$link?>"><i class="fa fa-download"></i> RENDER PDF</a>
    </center>
</div>
...
foreach ($jobs as $job):
	...
	$esta=Modules::run('jobs/getEstablishment',$district,$facility,$job_id,$thistype);
